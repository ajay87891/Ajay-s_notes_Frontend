...
function AddNote(props) {
  ...
  const handelClick =(e)=>{
    ...
    props.state(false)
    showAlert("Note Added", "Success", "visible")
  ...
  <button type="submit" disabled={note.title.length<5 || note.description.length <5} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={handelClick}>{t("note.5")}</button>
</form>
